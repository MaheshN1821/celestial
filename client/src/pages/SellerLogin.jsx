  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response.status === 200) {
        sessionStorage.setItem("sellerId", response.data.info._id);
        sessionStorage.setItem("sellerAccToken", response.data.accessToken);
        sessionStorage.setItem("sellerFirstName", response.data.info.firstName);
        sessionStorage.setItem("sellerLastName", response.data.info.lastName);
        Navigate("/seller");
      }
    } catch (err) {
      console.log(err);
